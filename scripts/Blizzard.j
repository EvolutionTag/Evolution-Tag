...
function PolarProjectionNoLoc takes location source,real dist,real angle returns location
...
function DisplayTextToForceNoForce takes force toForce,string message returns nothing
...
function DisplayTimedTextToForceNoForce takes force toForce,real duration,string message returns nothing
if(IsPlayerInForce(GetLocalPlayer(),toForce))then
...
function CreateNUnitsAtLocNoGroupNoLoc takes integer count,integer unitId,player whichPlayer,location loc,real face returns nothing
loop
set count=count-1
exitwhen count<0
...
function CreateNUnitsAtLocNoGroup takes integer count,integer unitId,player whichPlayer,location loc,real face returns nothing
loop
set count=count-1
exitwhen count<0
call CreateUnitAtLocSaveLast(whichPlayer,unitId,loc,face)
...
function ForGroupDestroyGroup takes group whichGroup,code callback returns nothing
...
function GroupAddGroupDestroySorce takes group sourceGroup,group destGroup returns nothing
...
function ForForceDestroyAfter takes force whichForce,code callback returns nothing
call ForForce(whichForce,callback)
...
function PingMinimapLocForForceExDestroyForce takes force whichForce,location loc,real duration,integer style,real red,real green,real blue returns nothing
call PingMinimapForForceEx(whichForce,GetLocationX(loc),GetLocationY(loc),duration,style,red,green,blue)
call DestroyForce(whichForce)
endfunction
...
function IssuePointOrderLocNoLoc takes unit whichUnit,string order,location whichLocation returns nothing
call IssuePointOrderLoc(whichUnit,order,whichLocation)
