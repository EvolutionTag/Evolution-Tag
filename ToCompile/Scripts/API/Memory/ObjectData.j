...
library APIMemoryObjectData
    ...
    function GetSpriteMatrixFloat takes integer pObject, integer offset returns real
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                return ReadRealFloat( pMatrix + offset )
            endif
        endif
    ...
    function SetSpriteMatrixFloat takes integer pObject, integer offset, real value returns real
        local integer pMatrix = 0
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                call WriteRealFloat( pMatrix + offset, value )
            endif
        endif
    ...
    function UpdateObjectColour takes integer pObject returns nothing // pObject = ConvertHandle( h )
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                call WriteRealMemory( pMatrix + 0x140, 0 )
                call WriteRealMemory( pMatrix + 0x13C, 0 )
                set flag = ReadRealMemory( pMatrix + 0x138 )

                if not IsFlagBitSet( flag, 0x800 ) then
                    call WriteRealMemory( pMatrix + 0x138, flag + 0x800 )
                endif
            endif
        endif
    endfunction
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                if alpha >= 0x00 and alpha <= 0xFF then
                    call fast_call_3( pSetObjectAlpha, pMatrix, alpha, 0 )
                endif
            endif
        endif
    endfunction
    ...
    function GetObjectColour takes integer pObject returns integer
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                return ReadRealMemory( pMatrix + 0x148 )
            endif
        endif
    ...
    function SetObjectColour takes integer pObject, integer colour returns nothing
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                ...
                call WriteRealMemory( pMatrix + 0x148, colour )
    ...
    function SetObjectAnimationByIndex takes integer pObject, integer index returns nothing
        ...
        if pObject > 0 then
            set pMatrix = ReadRealMemory( pMatrix + 0x28 )

            if pMatrix > 0 then
                call fast_call_3( pSetObjectAnimation, pMatrix, index, 0 )
            endif
        endif
    endfunction
	...
	function SetObjectPosition takes integer pObject, real x, real y, real z returns nothing
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                call WriteRealFloat( pMatrix + 0xC0, x )
                call WriteRealFloat( pMatrix + 0xC4, y )
                call WriteRealFloat( pMatrix + 0xC8, z )
            endif
        endif
	endfunction

	function GetObjectTimeScale takes integer pObject returns real
		return GetSpriteMatrixFloat( pObject, 0x190 )
	endfunction

	function SetObjectTimeScale takes integer pObject, real speed returns nothing
        call SetSpriteMatrixFloat( pObject, 0x190, speed )
	endfunction

	function GetObjectScale takes integer pObject returns real
		return GetSpriteMatrixFloat( pObject, 0xE8 )
	endfunction

	function SetObjectScale takes integer pObject, real scale returns nothing
        call SetSpriteMatrixFloat( pObject, 0xE8, scale )
	endfunction
	...
	function SetObjectScaleEx takes integer pObject, real x, real y, real z returns nothing
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                call WriteRealFloat( pMatrix + 0x108, x )
                call WriteRealFloat( pMatrix + 0x108, y )
                call WriteRealFloat( pMatrix + 0x108, z )
            endif
        endif
	endfunction
	...
	function ResetObjectMatrix takes integer pObject returns nothing
        ...
        if pObject > 0 then
            ...
            if pMatrix > 0 then
                call WriteRealFloat( pMatrix + 0x108, 1. )
                call WriteRealFloat( pMatrix + 0x10C, 0. )
                call WriteRealFloat( pMatrix + 0x110, 0. )
                call WriteRealFloat( pMatrix + 0x114, 0. )
                call WriteRealFloat( pMatrix + 0x118, 1. )
                call WriteRealFloat( pMatrix + 0x11C, 0. )
                call WriteRealFloat( pMatrix + 0x120, 0. )
                call WriteRealFloat( pMatrix + 0x124, 0. )
                call WriteRealFloat( pMatrix + 0x128, 1. )
            endif
        endif
	endfunction
...
//===========================================================================
function InitTrig_APIMemoryObjectData takes nothing returns nothing
    //set gg_trg_APIMemoryObjectData = CreateTrigger(  )
endfunction
//! endnocjass
