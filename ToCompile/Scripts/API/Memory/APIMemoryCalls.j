native MergeUnits 	takes integer qty, integer a, integer b, integer make returns boolean 	// reserved native for call 4 integer function and return BOOLEAN value
    ...
    library APIMemoryCalls
        ...
        function ReallocateCallMemory takes integer memory returns nothing
            ...
            set pfast_call_1  = memory +  0
            set pfast_call_2  = memory +  1000
            set pfast_call_3  = memory +  2000
            set pfast_call_4  = memory +  3000
            set pfast_call_5  = memory +  4000
            set pfast_call_6  = memory +  5000
            set pfast_call_7  = memory +  6000
            set pfast_call_8  = memory +  7000
            set pfast_call_9  = memory +  8000
            set pfast_call_10 = memory +  9000
            set pfast_call_11 = memory + 10000
            set pfast_call_12 = memory + 11000
            set pfast_call_13 = memory + 12000
    
            set pstd_call_1   = memory + 13000
            set pstd_call_2   = memory + 14000
            set pstd_call_3   = memory + 15000
            set pstd_call_4   = memory + 16000
            set pstd_call_5   = memory + 17000
            set pstd_call_6   = memory + 18000
    
            set pc_call_1     = memory + 19000
            set pc_call_2     = memory + 20000
            set pc_call_3     = memory + 21000
            set pc_call_4     = memory + 22000
            set pc_call_5     = memory + 23000
            set pc_call_6     = memory + 24000
    
            loop
                exitwhen i > 35
                call WriteRealMemory( memory + i * 1000, 0 )
                set i = i + 1
            endloop
        endfunction
    
        function AllocateCallMemory takes nothing returns nothing
            call ReallocateCallMemory( AllocateExecutableMemory( 35000 ) )
        endfunction
